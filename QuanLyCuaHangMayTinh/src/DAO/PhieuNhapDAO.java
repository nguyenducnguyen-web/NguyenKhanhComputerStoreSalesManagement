 ...
 */
public class PhieuNhapDAO implements GiaoDienDAO<PhieuNhap> {
    ...
    public ArrayList<PhieuTao> selectAllAccount(String acc) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String nguoiTao = rs.getString("nguoiTao");
                double tongTien = rs.getDouble("tongTien");
                PhieuTao p = new PhieuTao(maPhieu, thoiGianTao, nguoiTao, ChiTietPhieuNhapDAO.getInstance().selectAll(maPhieu), tongTien);
                ketQua.add(p);
            }
        } catch (Exception e) {
            // TODO: handle exception
